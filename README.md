...
| `.github/` | Continuous integration, and the bot that maintains its pins. It runs the same entry points as the commit hook and `just verify` — `just check` once, and `just test` once per interpreter the engine claims to support, the version floor included — so CI and the local runners cannot disagree about what "green" means, and the floor is a claim somebody executes. The one deliberate exception is half of the guard gate below: whether the settings still describe an intact boundary is asked everywhere, while whether the guard itself answers is asked only where the guard exists. `dependabot.yml` keeps the workflow's commit-pinned actions current. |
