...
| `src/frisk/` | The engine: the code that parses a command line and judges what it finds. Standard library only, zero dependencies, and written to run on Python 3.9 — the version the oldest supported platform ships, because an engine the operator's interpreter cannot parse would make the hook fail open. It carries the CLI; today that CLI answers `--version` and tells `explain` that there is no engine yet. |
| `tests/` | The engine's own suite. It runs with nothing installed — `PYTHONPATH=src python3 -m unittest discover -s tests -t tests`, which is what `just test` does — and that is what lets CI run it on an interpreter at the version floor. |
| `pyproject.toml` | Packaging metadata for the repository-installable engine and its `frisk` console script (the door a project's CI uses), plus the configuration of the Python checks. Installing is *not* how the plugin will deliver frisk: that channel is build-free. |
| `justfile`, `scripts/` | The documented entry points: `just setup`, `just check [all\|changed]`, `just test`, `just verify`. `just setup` also installs the engine into the local environment, so the console script is exercised rather than assumed. The scripts are thin glue; the checks themselves are configured, not written. The one check written here rather than configured is `scripts/check_frontmatter.py`, a few lines asserting that every skill and agent definition still loads. |
...
| `.github/` | Continuous integration, and the bot that maintains its pins. It runs the same entry points as the commit hook and `just verify` — `just check` and `just test`, one job each, plus a third that runs `just test` again on an interpreter at the engine's version floor — so CI and the local runners cannot disagree about what "green" means, and the floor is a claim somebody executes. The one deliberate exception is half of the guard gate below: whether the settings still describe an intact boundary is asked everywhere, while whether the guard itself answers is asked only where the guard exists. `dependabot.yml` keeps the workflow's commit-pinned actions current. |
