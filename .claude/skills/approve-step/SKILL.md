...
6. **Report, then attempt the push.** Show the step summary and what the
   ...
   Then attempt two pushes, in this order:

   ```sh
   git push --follow-tags
   git remote get-url backup >/dev/null 2>&1 \
       && git push backup refs/backups/bash-guard \
       || echo "no backup remote: the guard ref stays local"
   ```
   ...
   locally is invisible to everything reading the remote. The second
   push carries the quarantined guard's backup ref (`CLAUDE.md` rule 1)
   to the operator's private backup remote — **`backup`, never
   `origin`**, which is public. Where that remote does not exist, say so
   in the close summary and attempt nothing; `.claude/docs/guard-record.md`
   § "Restore, and the backup ref" records its current absence.
